...
const Product = () => {
  ...
  useEffect(() => {
    if (productData) {
      setImgPoster(productData.image?.[0] || null);
    }
  }, [productData]);
    ...
    <div className="border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100">
      ...
      <div className="flex gap-12 sm:gap-12 flex-col sm:flex-row">
          ...
          <div className="flex sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-between sm:justify-normal sm:w-[18.7%] w-full">
            {productData.image.map((item, index) => (
              <img
                src={item}
                key={index}
                onClick={() => setImgPoster(item)}
                className="w-[24%] sm:w-full sm:mb-3 flex-shrink-0 cursor-pointer"
              />
            ))}
          </div>
          <div className="w-full sm:w-[80%]">
            <img className="w-full h-auto" src={imgPoster} />
          </div>
        </div>
        ...
        <div className="flex-1">
          ...
          <button
            ...
            className="bg-black text-white px-8 py-3 cursor-pointer rounded text-sm active:bg-gray-700"
          >
            ADD TO CART
          </button>
      ...
      <div className="mt-20">
        ...
        <div className="flex flex-col gap-4 border border-gray-300 px-6 py-6 text-sm text-gray-500">
          <p>
            An e-commerce website is an online platform that facitates the
